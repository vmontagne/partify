...
import {AlbumTracks, ArtistAlbums, SearchResponse} from '../shared/spotifyType'
...
class Spotify {
    ...
    private tokenIsValid() {
        return this.accessToken && this.tokenCreatedAt && this.tokenCreatedAt.plus({seconds: this.TOKEN_DURATION - 10}) > DateTime.now()
    }
    ...
    async search(query:string): Promise<SearchResponse> {
        const data = await this.fetch<SearchResponse>('/search?' + new URLSearchParams({
            q: query,
